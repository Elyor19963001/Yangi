...
function plannerProfile(){const active=(selector,attr)=>document.querySelector(selector)?.dataset?.[attr]||null;const legacyInterests=[...document.querySelectorAll('[data-interest].active')].map(x=>x.dataset.interest).filter(Boolean);const budget=Number($('budget')?.value);const ai=window.tfeAiProfile&&typeof window.tfeAiProfile==='object'?window.tfeAiProfile:{};const interests=Array.isArray(ai.interests)&&ai.interests.length?ai.interests:legacyInterests;return {interests,pace:ai.pace||active('[data-pace].active','pace')||'normal',transport:ai.transport||active('[data-transport].active','transport')||'mixed',low_walking:typeof ai.low_walking==='boolean'?ai.low_walking:Boolean(document.querySelector('[data-low-walking].active')),wheelchair_accessible:typeof ai.wheelchair_accessible==='boolean'?ai.wheelchair_accessible:Boolean(document.querySelector('[data-wheelchair].active')),own_vehicle:typeof ai.own_vehicle==='boolean'?ai.own_vehicle:Boolean(document.querySelector('[data-own-car].active')),children_count:ai.children_count!==undefined?Math.max(0,Number(ai.children_count)||0):Math.max(0,Number($('childrenCount')?.value)||0),seniors_count:ai.seniors_count!==undefined?Math.max(0,Number(ai.seniors_count)||0):Math.max(0,Number($('seniorCount')?.value)||0),preferred_start_time:ai.preferred_start_time||$('preferredStartTime')?.value||null,preferred_end_time:ai.preferred_end_time||$('preferredEndTime')?.value||null,origin_country:ai.origin_country||$('originCountry')?.value?.trim()||null,budget_uzs:Number.isFinite(Number(ai.budget_uzs))&&Number(ai.budget_uzs)>0?Number(ai.budget_uzs):(Number.isFinite(budget)&&budget>0?budget:null)}}
